...
func NewPublicKey(in []byte) (*PublicKey, error) {
	...
	k, err := sr25519.NewPublicKey(buf)
	if err != nil {
		return nil, err
	}
...
func (k *PublicKey) VrfVerify(t *merlin.Transcript, out [VrfOutputLength]byte, proof [VrfProofLength]byte) (bool, error) {
	...
	newOut, err := sr25519.NewOutput(out)
	if err != nil {
		return false, err
	}

	return k.key.VrfVerify(t, newOut, p)
}
...
func AttachInput(output [VrfOutputLength]byte, pub *PublicKey, t *merlin.Transcript) (*sr25519.VrfInOut, error) {
	out, err := sr25519.NewOutput(output)
	if err != nil {
		return nil, err
	}
	return out.AttachInput(pub.key, t), nil
}
