...
import (
	...
	"github.com/lambdanew/ln/pkg/crypto"
)
...
type PrivateKey ed.PrivateKey

// PublicKey is the ed25519 Public Key
type PublicKey ed.PublicKey
...
func NewKeypair(priv ed.PrivateKey) *Keypair {
	pubkey := PublicKey(priv.Public().(ed.PublicKey))
...
// NewKeypairFromPrivate returns a ed25519 Keypair given a *ed.PrivateKey
...
func NewPrivateKey(in []byte) (*PrivateKey, error) {
	...
	priv := PrivateKey(ed.PrivateKey(in))
	return &priv, nil
}
...
func Verify(pub *PublicKey, msg, sig []byte) (bool, error) {
	...
	return ed.Verify(ed.PublicKey(*pub), msg, sig), nil
}
...
func (kp *Keypair) Sign(msg []byte) ([]byte, error) {
	return ed.Sign(ed.PrivateKey(*kp.private), msg), nil
}
